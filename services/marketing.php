...
switch ($method) {
    case 'GET':
        $sql = "CALL marketing_get_important()";
    ...
    case 'POST': // Insert
        ...
        $sql = "INSERT INTO marketing (" . implode(", ", $columns) . ") VALUES (" . implode(", ", $placeholders) . ")";
        ...
        if ($stmt->execute()) {
            echo json_encode(["status" => 200, "message" => "Marketing data added successfully"]);
        } else {
    ...
    case 'PUT':
        ...
        $updateQuery = "UPDATE marketing SET " . implode(", ", $updateFields) . " WHERE id = ?";
        $params[] = $id;
        ...
        if ($stmt->execute()) {
            echo json_encode(["status" => 200, "message" => "Marketing data updated successfully"]);
        } else {
            http_response_code(409);
            echo json_encode(["status" => 409, "error" => $stmt->error]);
        }
