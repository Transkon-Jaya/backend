...
try {
    // Transaksi hanya untuk POST, PUT, DELETE
    ...
    if ($method === 'POST') {
        ...
        if (empty($input['ta_id'])) {
            ...
            $stmt->execute();
            $res = $stmt->get_result();
            $lastId = $res->fetch_assoc()['last_id'] ?? null;
    ...
    if ($method === 'PUT') {
        ...
        if (!$set) throw new Exception("Tidak ada data untuk diupdate", 400);

        $set[] = "updated_by = ?";
        $params[] = $currentName; $types .= 's';
    ...
    if ($method === 'GET') {
        ...
        $total = $conn->query("SELECT FOUND_ROWS() as total")->fetch_assoc()['total'] ?? 0;
    ...
    if (in_array($method, ['POST', 'PUT', 'DELETE']) && $conn->errno) {
        $conn->rollback();
    }
    exit;
} finally {
